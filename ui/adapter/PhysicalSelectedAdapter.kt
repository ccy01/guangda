 ...
 */
class PhysicalSelectedAdapter(val context: Context, val dataBeanList: MutableList<PhysicalBean>) : RecyclerView.Adapter<BaseViewHolder>() {
    ...
    override fun onBindViewHolder(holder: BaseViewHolder, position: Int) {
        when (getItemViewType(position)) {
            PhysicalBean.PARENT_ITEM -> {
                val parentViewHolder = holder as ParentSelectedViewHolder
                parentViewHolder.bindView(dataBeanList[position], itemClickListener)
            }

            PhysicalBean.CHILD_ITEM -> {
                val childViewHolder = holder as ChildSelectedViewHolder
