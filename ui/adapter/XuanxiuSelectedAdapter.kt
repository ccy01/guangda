 ...
 */
class XuanxiuSelectedAdapter(val context: Context, val dataBeanList: MutableList<XuanXiuSelectedBean>) : RecyclerView.Adapter<BaseViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder {
        var view: View
        when (viewType) {
            XuanXiuSelectedBean.PARENT_ITEM -> {
                view = mInflater!!.inflate(R.layout.xuanxiu_group_selected, parent, false)
                return XParentSelectedViewHolder(context, view)
            }

            XuanXiuSelectedBean.CHILD_ITEM -> {
                view = mInflater!!.inflate(R.layout.xuanxiu_child_selected, parent, false)
                return XChildSelectedViewHolder(context, view)
            }
            else -> {
                view = mInflater!!.inflate(R.layout.xuanxiu_group_selected, parent, false)
                return XParentSelectedViewHolder(context, view)
    ...
    override fun onBindViewHolder(holder: BaseViewHolder, position: Int) {
        when (getItemViewType(position)) {
            PhysicalBean.PARENT_ITEM -> {
                val parentViewHolder = holder as XParentSelectedViewHolder
                parentViewHolder.bindView(dataBeanList[position], itemClickListener)
            }

            PhysicalBean.CHILD_ITEM -> {
                val childViewHolder = holder as XChildSelectedViewHolder
    ...
    val itemClickListener = object : ItemClickListener {
        override fun onExpandChildren(bean: BaseBean) {
            val child = (bean as XuanXiuSelectedBean).copy(type = 1)
        ...
        override fun onHideChildren(bean: BaseBean) {
            val position = getCurrentPosition((bean as XuanXiuSelectedBean).ID)//确定当前点击的item位置
     ...
     * @param position
     */
    fun add(bean: XuanXiuSelectedBean, position: Int) {
