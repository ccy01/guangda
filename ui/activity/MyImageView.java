...
public class MyImageView extends ImageView {
    ...
    private void drawTextAtBitmap() {
        Bitmap bitmap = Bitmap.createScaledBitmap(((BitmapDrawable)getDrawable()).getBitmap(),mWidth,mHeight,true);
        Bitmap newbit = Bitmap.createBitmap(mWidth, mHeight, Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(newbit);
        Paint paint = new Paint(); // 在原始位置0，0插入原图
        canvas.drawBitmap(bitmap, 0, 0, paint);
        paint.setColor(Color.parseColor("#000000"));
        paint.setTextSize(48); // 在原图指定位置写上字
        if (text != null) {
            float sizeText = paint.measureText(text);
            canvas.drawText(text, mWidth / 2 - sizeText / 2, mHeight / 2, paint);
        }
        canvas.save(Canvas.ALL_SAVE_FLAG); // 存储
        canvas.restore();
        this.setImageBitmap(newbit);
    }
